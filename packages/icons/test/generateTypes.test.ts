...
suite("generateIconTypeDeclarations", () => {
	suite("Basic Functionality", () => {
		test("generates type declarations for empty array", () => {
			...
			assert.ok(
				result.includes('export type { IconData } from "./types.d.ts"'),
			);
		...
		test("generates type declarations for multiple icons", () => {
			...
			assert.ok(
				result.includes('export type IconName = "home" | "user" | "settings"'),
			);
		});
	...
	suite("Edge Cases", () => {
		...
		test("handles very long icon names", () => {
			const longName =
				"very-long-icon-name-with-many-characters-and-descriptive-text";
	...
	suite("Output Structure", () => {
		...
		test("maintains proper indentation", () => {
			...
			assert.ok(moduleLine?.startsWith("declare module"));

			// Check that type reference is indented
			const typeRefLine = lines.find((line) =>
				line.includes("export type { IconData }"),
			);
			assert.ok(typeRefLine?.startsWith("  export type { IconData }"));

			// Check that IconName type is indented
			const iconNameLine = lines.find((line) =>
				line.includes("export type IconName"),
			);
			assert.ok(iconNameLine?.startsWith("  export type IconName"));
		});
	});
