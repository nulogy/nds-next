...
suite("parseSvg", () => {
  ...
  suite("Smoke Tests", () => {
    test("parses check.svg from assets", async () => {
      const svgData = await readFile(path.join(ASSETS_DIR, "check.svg"), "utf-8");
      ...
      assert.ok(firstPath &&firstPath.length > 0, "Path data should not be empty");
    });

    test("parses home.svg from assets", async () => {
      const svgData = await readFile(path.join(ASSETS_DIR, "home.svg"), "utf-8");
      const { viewBox, path: svgPath } = parseSvg(svgData);
    ...
    test("parses settings.svg from assets", async () => {
      const svgData = await readFile(path.join(ASSETS_DIR, "settings.svg"), "utf-8");
      const { viewBox, path: svgPath } = parseSvg(svgData);
    ...
    test("parses user.svg from assets", async () => {
      const svgData = await readFile(path.join(ASSETS_DIR, "user.svg"), "utf-8");
      const { viewBox, path: svgPath } = parseSvg(svgData);
