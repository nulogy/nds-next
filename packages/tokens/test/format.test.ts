...
suite('formatTokens function', () => {
  ...
  test('should generate correct CSS variable declarations', () => {
    assert.ok(result.cssVars.includes(`--nds-${device}-${folderName}-primary: #ffffff;`))
    assert.ok(result.cssVars.includes(`--nds-${device}-${folderName}-nested-tertiary: #ff00ff;`))
  })

  test('should generate correct JS export statements', () => {
    assert.ok(result.jsExports.includes(`export const ${toUpper(device)}_${toUpper(folderName)}_PRIMARY = "#ffffff";`))
    assert.ok(result.jsExports.includes(`export const ${toUpper(device)}_${toUpper(folderName)}_NESTED_TERTIARY = "#ff00ff";`))
  })
  ...
  test('should include the correct export in the JS output', () => {
    assert.ok(result.js.includes(`export const ${toUpper(device)}_${toUpper(folderName)}_PRIMARY = "#ffffff";`))
  })
})
